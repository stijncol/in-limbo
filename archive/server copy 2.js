...
<script>
  ...
  function ditherImage(img, thumb) {
    ...
    // Convert to grayscale with green tint, then Floyd-Steinberg dither
    for (let i = 0; i < d.length; i += 4) {
      d[i] = d[i] * 0.299 + d[i+1] * 0.587 + d[i+2] * 0.114;
      d[i+1] = d[i];
      d[i+2] = d[i];
    }
    ...
    for (let i = 0; i < d.length; i += 4) {
      const v = d[i];
      d[i] = Math.min(255, v * 0.75 + 60);      // R
      d[i+1] = Math.min(255, v * 0.85 + 80);    // G — boosted
      d[i+2] = Math.min(255, v * 0.7 + 60);      // B
    }
